...
fn game1(start_pos: &Vec<u64>) -> u64 {
...
fn game2(start_pos: &Vec<u64>) -> Vec<u64> {
    fn get_num_winning(
        cur_pos: &(u64, u64),
        scores: &(u64, u64),
        cur_player: usize,
        ...
        for i in 1..=3 {
            for j in 1..=3 {
                for k in 1..=3 {
                    let steps = i + j + k;
                    let mut pos = steps + if cur_player == 1 {cur_pos.1} else {cur_pos.0};
                    pos = (pos - 1) % 10 + 1;
                    let wins = match cur_player {
                        0 => get_num_winning(&(pos, cur_pos.1),
                                            &(scores.0 + pos, scores.1),
                                            1,
                                            mem),
                        1 => get_num_winning(&(cur_pos.0, pos),
...
pub(crate) fn solution() {
    ...
    let start_pos: Vec<u64> = lines.iter()
